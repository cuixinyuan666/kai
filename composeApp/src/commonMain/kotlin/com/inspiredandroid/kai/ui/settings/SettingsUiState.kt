...
data class SettingsUiState(
    ...
    val serviceSortReversed: Boolean = false,
    val isDefaultInteractiveMode: Boolean = false,
    // 模型基准测试（一键测试所有大模型）
    val modelBenchmarks: ImmutableList<ModelBenchmark> = persistentListOf(),
    val isBenchmarkRunning: Boolean = false,
    /** 0..1 当前测试进度 */
    val benchmarkProgress: Float = 0f,
    /** 正在测试的模型标签（并行时显示“完成 n/total：模型”） */
    val benchmarkCurrentLabel: String = "",
    /** 已完成的模型数（并行进度用） */
    val benchmarkDoneCount: Int = 0,
    val benchmarkDone: Boolean = false,
    /** 测试结束后的结果统计文本 */
    val benchmarkSummary: String = "",
    // OpenCode 终端 thinking/plan/build
    val openCodeTerminalThinking: Boolean = false,
    val openCodeTerminalMode: String = "build",
)
