...
internal fun GeneralContent(uiState: SettingsUiState, actions: SettingsActions) {
    StaggeredSettingsColumns(
        start = {
            ...
            SettingsCard {
                InteractiveUiDefaultToggle(
                    isDefault = uiState.isDefaultInteractiveMode,
                    onToggle = actions.onToggleDefaultInteractiveMode,
                )
            }
            SettingsCard {
                ThemeModePicker(
...
@Composable
private fun InteractiveUiDefaultToggle(
    isDefault: Boolean,
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        ToggleableHeadline(
            title = "默认启用互动 UI",
            description = "新对话默认以互动 UI 模式开始（驱动工具调用循环，渲染可交互卡片）。关闭则仅在聊天空态手动开启。",
            checked = isDefault,
